...
def display_value(ops: List[Operand], symbol_table, expected_size, idr):
    vals = [op.resolve_value(symbol_table, idr) for op in ops]
    if expected_size == 1:
        return ""
...
def second_pass(
    ...
    addrformat = AddressFormat.MOVABLE

    ### Формирование заголовка
    try:
        start_line_inx, start_line = auxalirity_table[0]
        if start_line_inx != 0:
            addrformat = AddressFormat.ABSOLUTE
    ...
    for dlta, (location, line) in zip(dltas, auxalirity_table):
        lineErr = lambda msg: errors.append(f" [{location:06X}]: {msg}")
        ...
        try:
            if mnemonic in op_table_dict:
                ...
                machine_code.append(
                    f"T {location:06X} {dlta} {op_adr_code:02x}{display_value(ops, symbol_table, expected_size, location+dlta)}"
                )
                if addrformat == AddressFormat.ABSOLUTE and addrtype == AddressType.RELATIVE:
                    lineErr("Относительная адресация в абсолютном формате")
                if address_type(ops) == AddressType.DIRECT:
                    modification_table.append(location)
        except KeyError as err:
            lineErr(str(err))

    ### Формирование модификаторов
    for location in modification_table:
        machine_code.append(f"M {location:06X}")
