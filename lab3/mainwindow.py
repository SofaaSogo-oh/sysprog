...
def parsed_lines_to_simple_table(
    table_widget, code_op_table, parsed_lines_with_addresses: List[ParsedLine]
):
    table_widget.clear()
    ...
    for row, (address, parsed_line, _) in enumerate(parsed_lines_with_addresses):
        mnemonic = parsed_line.command.mnemonic
        if mnemonic == "[ESECT]":
            continue
        address_item = QTableWidgetItem(
            "" if mnemonic in ["EXTREF", "EXTDEF"] else f"0x{address:06X}"
        )
...
def write_modification_table(table_widget: QTableWidget, modification_table):
    table_widget.clear()
    table_widget.setRowCount(0)
    table_widget.setColumnCount(3)
    table_widget.setHorizontalHeaderLabels(['Адрес', 'Имя', 'Секция'])
    table_widget.setRowCount(len(modification_table))
    for row, (loc, name, section) in enumerate(modification_table):
        loc_item = QTableWidgetItem(f"{loc:06X}")
        name_item = QTableWidgetItem(name)
        section_item = QTableWidgetItem(section)

        table_widget.setItem(row, 0, loc_item)
...
class MainWindow(QMainWindow):
    ...
    def update_ui_state(self):
        ...
        self.ui.secondPass.setEnabled(
            self.first_pass_successful and not self.second_pass_successful
        )

        # Делаем красивости
        if self.second_pass_successful:
            self.ui.secondPass.setStyleSheet(
    ...
    def first_pass(self):
        ...
        auxiliraty_table, symbol_table, first_pass_errors = first_pass_simple_dict(
            parsed_lines, op_table, method
        )
        ...
        if first_pass_errors:
            ...
            if auxiliraty_table:
                parsed_lines_to_simple_table(
                    self.ui.helpTable, op_table, auxiliraty_table
                )
            if symbol_table:
                fill_symbol_table_sorted_by_address(
                    self.ui.symbolicNameTable, symbol_table
                )
    ...
    def second_pass(self):
        if (
            self.auxaliraty_table is None
            or self.symbol_table is None
            or self.op_table is None
        ):
        ...
        machine_code, second_pass_errors, modification_table = second_pass(
            self.auxaliraty_table, self.symbol_table, self.op_table
        )
        ...
        if second_pass_errors:
            ...
            if machine_code:
                ...
                write_modification_table(self.ui.modTable, modification_table)
